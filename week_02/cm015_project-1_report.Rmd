...
- the `'item'` argument creates `results_list` as a "list of `r length(results_list)`" --- each observation in the data frame `item` column becomes one entry in the list. 

To see the structure of these objects for yourself, in the Console, type

- `glimpse(results_df)` to see the data frame
- `glimpse(results_list)` to see the list of `r length(results_list)`

Note that each item in `results_list` (`r results_list[[1]]$item`, `r results_list[[2]]$item`, etc) is a  data frame containing one row from the original data frame with columns: `item` (a string), `num` (a number), and `unit` (another string).
...
- \`r sensor\_model\` is replaced in the output document by its value, "`r sensor_model`" 
- \`r force\_limit\` is replaced in the output document by its value, "`r force_limit`"
- \`r force\_unit\` is replaced in the output document by its value, "`r force_unit`"
...
- \`r input\_unit\` is replaced in the output document by its value, "`r input_unit`" 
- \`r output\_unit\` is replaced in the output document by its value, "`r output_unit`"

`r code_icon`

```{r 113-11, fig.cap = 'Figure 1. Load cell calibration test setup'}
...
```{r 113-13, fig.cap = 'Figure 2. Load cell calibration curve'}
