...
{%% slide -------------------------------------------
\renewcommand\foottext{Steven Goodman, Daniele Fanelli, and John Ioannidis (2016) \href{http://stm.sciencemag.org/content/8/341/341ps12}{What does research reproducibility mean?}, {\em Science Translational Medicine}, 8(341).}

\begin{frame}[fragile,c]{New terminology is proposed to clarify  meaning}

\bigskip
\hspace{-1em}\begin{tabular}{@{} p{1.8in} p{3.2in}}
& \\
{\em methods reproducibility} & same data, same analysis, same results ({\em transparency})\\
& \\
{\em results reproducibility} & replication: corroborative results in a new study\\
& \\
{\em inferential reproducibility} & independently replicate or re-analyze data and draw draw qualitatively similar conclusions
\end{tabular}
\end{frame}
}%%
...
\begin{frame}[fragile, c]{Our focus is on {\em methods reproducibility}}
...
\column{0.6\textwidth}
Explicitly linking the report, paper, or talk to the data and scripts that generate the findings
