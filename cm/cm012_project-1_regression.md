...
In our linear model, `lm()`, the coefficient of the \(x^0\) term is labeled *Intercept*; the coefficient of the \(x^1\) term is labeled the *input\_lb* coefficient, i.e., the coefficient of \(x^1\). This is our slope. (Higher-order polynomial-fits would have additional coefficients, of course, for \(x^2, x^3\), etc.)
