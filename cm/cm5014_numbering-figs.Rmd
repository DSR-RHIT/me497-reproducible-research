...
The function names on the left, `table_id` and `figure_id`, are arbitrary. 
...
We create *table captions* using `table_id()`. The `name` argument is a unique string to identify the table prefix and `caption` string.
...
The table number (in this case, Table 1) is determined by the order of appearance of the `table_id()` function in the script. This is its first use, so the table number is "1". 
...
> An excerpt of data from our fabulous experiment is shown in `r table_id("a_table", display = "cite")`. 

To construct this sentence, we an inline code chunk and the function  `table_id()` with an added argument `display = "cite"` to print the table number but not the caption. 
...
The `kable()` function has a `caption` argument we use to invoke the `table_id()` function.  
...
In docx output, table captions are formatted with the *Table Caption* style. 
...
> More amazing results are shown in `r table_id("another_table", display = "cite")`. 
...
Create my first figure caption using  `figure_id()`. 

```{r results = 'hide'}
figure_id(name = "one_figure", caption = "An auto-numbered figure caption.")
```

The figure number (in this case, Figure 1) is determined by the order of appearance of the `figure_id()` function in the script. This is its first use, so the figure number is "1". 
...
> More amazing results are shown in `r figure_id("one_figure", display = "cite")`. 
...
```{r fig.cap = figure_id("one_figure"), fig.height = 3, eval = FALSE}
...
`r figure_id("another_figure", display = "cite")`. 
...
```{r fig.cap = figure_id("another_figure"), fig.height = 3}
ggplot(data = cars, aes(x = speed, y = dist)) + 
	geom_point() + 
