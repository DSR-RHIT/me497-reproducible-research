...
For substantive projects, students have found it useful to separate raw data from tidy data by having two data directories. 

- The `raw` folder is for data as first obtained in whatever form it happens to be. 
- Raw data are never operated on manually.  
- R scripts operate on raw data to create tidy data frames which are written to the `tidy` directory.  
- R scripts operate on the tidy data for analysis and to produce graphs. 


Some practitioners like this arrangement because they can omit the raw folder from version control because the raw data files are so large. 
...
If you prefer of course you could use this directory structure for all your projects. 
