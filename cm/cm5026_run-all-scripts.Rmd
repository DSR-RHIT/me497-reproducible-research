...
When you want to run an entire project's worth of R markdown scripts from beginning to end, the "proper" approach is to use *makefiles*. Makefiles save time by only executing those the scripts that have changed since the last time the makefile was run. 
