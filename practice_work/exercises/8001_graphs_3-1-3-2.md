...
Launch your `practice_work` project
  ...
  - save it in the `exercises` folder, selecting a deliberate filename,
    for example, I’m using `8001_graphs_3-1-3-2.Rmd`. (All my exercises
    will be in the 8000 series. Your mileage may vary.)

After your name and date, add the following in a code chunk. The knitr
options set here are to - set the knitr root directory one level up to
match the project working directory - show the source code and output
from a code chunk in a single block
...
book in the Rmd file. Knit the file regularly and compare your results
to those shown in the book. The goal is to learn from the activity so
you can work the exercises.
